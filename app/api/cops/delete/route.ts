...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (authError) {
      console.error('[CoP 삭제] 인증 오류:', authError)
    ...
    console.log(`[CoP 삭제] CoP 삭제 시작: ${copIds.length}개`)
    ...
    if (!SUPABASE_SERVICE_ROLE_KEY) {
      console.error('[CoP 삭제] SUPABASE_SERVICE_ROLE_KEY가 설정되지 않았습니다.')
    ...
    if (deleteError) {
      console.error('[CoP 삭제] 오류:', deleteError)
      return NextResponse.json({ error: '삭제 실패: ' + deleteError.message }, { status: 500 })
    }

    console.log(`[CoP 삭제] CoP 삭제 완료: ${copIds.length}개`)
