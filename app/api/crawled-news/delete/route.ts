import { NextRequest, NextResponse } from 'next/server'
import { createClient } from '@supabase/supabase-js'

// crawled_news 항목 삭제
export async function DELETE(request: NextRequest) {
  try {
    // Authorization 헤더에서 토큰 확인
    const authHeader = request.headers.get('authorization')
    
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return NextResponse.json({ error: '인증 토큰이 필요합니다.' }, { status: 401 })
    }

    const token = authHeader.substring(7)

    // 토큰으로 Supabase 클라이언트 생성
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
      {
        global: {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        },
      }
    )

    // 사용자 정보 확인
    const { data: { user }, error: userError } = await supabase.auth.getUser(token)

    if (userError || !user) {
      console.error('[삭제] 인증 오류:', userError)
      return NextResponse.json({ error: '인증 오류: ' + (userError?.message || '사용자 정보를 가져올 수 없습니다.') }, { status: 401 })
    }

    // 관리자 확인
    const ADMIN_EMAIL = process.env.NEXT_PUBLIC_ADMIN_EMAIL || ''
    if (user.email !== ADMIN_EMAIL) {
      return NextResponse.json({ error: '관리자 권한이 필요합니다.' }, { status: 403 })
    }

    const body = await request.json()
    const { crawledNewsIds, ids }: { crawledNewsIds?: string[], ids?: string[] } = body

    // crawledNewsIds 또는 ids 중 하나를 사용
    const idsToDelete = crawledNewsIds || ids

    if (!idsToDelete || !Array.isArray(idsToDelete) || idsToDelete.length === 0) {
      return NextResponse.json({ error: '삭제할 항목을 선택해주세요.' }, { status: 400 })
    }

    console.log(`[삭제] ${idsToDelete.length}개 항목 삭제 시작`)

    // 삭제 전에 존재하는 항목 확인
    const { data: existingItems, error: checkError } = await supabase
      .from('crawled_news')
      .select('id')
      .in('id', idsToDelete)

    if (checkError) {
      console.error('[삭제] 조회 오류:', checkError)
      return NextResponse.json({ error: '조회 실패: ' + checkError.message }, { status: 500 })
    }

    const existingIds = existingItems?.map(item => item.id) || []
    console.log(`[삭제] 존재하는 항목: ${existingIds.length}개 / 요청한 항목: ${idsToDelete.length}개`)

    if (existingIds.length === 0) {
      return NextResponse.json({ 
        success: true, 
        deleted: 0,
        message: '삭제할 항목이 없습니다.'
      })
    }

    // crawled_news에서 삭제 (관리자는 모든 항목 삭제 가능)
    const { data: deletedData, error: deleteError } = await supabase
      .from('crawled_news')
      .delete()
      .in('id', existingIds)
      .select()

    if (deleteError) {
      console.error('[삭제] 오류:', deleteError)
      return NextResponse.json({ error: '삭제 실패: ' + deleteError.message }, { status: 500 })
    }

    const deletedCount = deletedData?.length || 0
    console.log(`[삭제] ${deletedCount}개 항목 삭제 완료`)

    return NextResponse.json({
      success: true,
      deleted: deletedCount,
      requested: idsToDelete.length,
      found: existingIds.length,
    })
  } catch (error: any) {
    console.error('[삭제] 오류:', error)
    return NextResponse.json(
      { error: '삭제 중 오류가 발생했습니다.', details: error?.message || String(error) },
      { status: 500 }
    )
  }
}
